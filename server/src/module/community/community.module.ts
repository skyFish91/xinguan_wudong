...
export class CommunityService {
  ...
  async toggleTopicFollow(userId: number, topicId: number) {
    ...
    const exists = await this.favoriteRepo.findOneBy({ userId, bizType: 'topic', bizId: topicId });
    if (exists) {
    ...
    await this.favoriteRepo.save(this.favoriteRepo.create({ userId, bizType: 'topic', bizId: topicId }));
    topic.followCount += 1;
    await this.topicRepo.save(topic);

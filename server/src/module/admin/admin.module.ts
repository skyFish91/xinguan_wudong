...
export class AdminService {
  ...
  async dashboard() {
    ...
    const byStatus = await this.orderRepo
      ...
      .groupBy('o.status')
      .getRawMany();
    const pv = await this.visitLogRepo.count();
    const todayPv = await this.visitLogRepo.createQueryBuilder('v').where('v.created_at >= :t', { t: today }).getCount();
    const uv = await this.visitLogRepo.createQueryBuilder('v').select('COUNT(DISTINCT v.user_id)', 'uv').getRawOne();
    const merchantCount = await this.merchantRepo.count();
    const pendingApplies = await this.applyRepo.countBy({ status: 0 });
    // 近 7 天订单趋势
    ...
    const trend = await this.orderRepo
      ...
      .where('o.created_at >= :t AND o.status >= 1 AND o.status NOT IN (5,7)', { t: sevenDaysAgo })

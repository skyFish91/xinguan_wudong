    ...
    <el-tabs v-model="tab" @tab-change="reload">
      ...
      <el-tab-pane label="账单明细" name="records">
        <div class="toolbar">
          ...
          <el-tag size="small" type="info" effect="plain">共 {{ recordTotal }} 条</el-tag>
        </div>
        <el-table :data="records" v-loading="recordsLoading" stripe>
          ...
          <el-table-column label="订单金额" width="120">
            <template #default="{ row }"><span class="money">¥{{ row.orderAmount }}</span></template>
          </el-table-column>
          <el-table-column label="佣金率" width="90">
            <template #default="{ row }">{{ row.commissionRate }}%</template>
          </el-table-column>
          <el-table-column label="平台佣金" width="120">
            <template #default="{ row }"><span class="money">¥{{ row.commission }}</span></template>
          </el-table-column>
          <el-table-column label="商家收入" width="120">
            <template #default="{ row }"><span class="money income">¥{{ row.merchantIncome }}</span></template>
          </el-table-column>
          <el-table-column label="结算状态" width="100">
            <template #default="{ row }">
              <el-tag :type="row.settleStatus === 1 ? 'success' : 'warning'" size="small" effect="dark">
        ...
        <EmptyState v-if="!recordsLoading && records.length === 0" type="data" description="暂无账单记录" />
        <el-pagination
      ...
      <el-tab-pane label="结算单" name="settlements">
        ...
        <el-table :data="settlements" v-loading="settleLoading" stripe>
          ...
          <el-table-column label="结算金额" width="120">
            <template #default="{ row }"><span class="money income">¥{{ row.amount }}</span></template>
          </el-table-column>
          ...
          <el-table-column label="操作" width="110" fixed="right">
            <template #default="{ row }">
              <el-button v-if="row.status === 0" size="small" type="success" link @click="confirmSettle(row)">确认打款</el-button>
              <span v-else class="muted">—</span>
            </template>
          </el-table-column>
        </el-table>
        <EmptyState v-if="!settleLoading && settlements.length === 0" type="data" description="暂无结算单" />
        <el-pagination
          layout="prev, pager, next, total"
          :total="settleTotal"
          :page-size="10"

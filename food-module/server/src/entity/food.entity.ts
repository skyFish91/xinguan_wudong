...
export class RestaurantEntity {
  ...
  @Column({ name: 'avg_price',  type: 'decimal', precision: 10, scale: 2, default: 0 })
  avgPrice: number;
...
/** 食模块-评价（餐厅/农产品） */
@Entity('t_food_review')
export class FoodReviewEntity {
  @PrimaryGeneratedColumn({ type: 'bigint', unsigned: true })
  id: number;
...
/** 食模块-收藏（餐厅/农产品） */
@Entity('t_food_favorite')
export class FoodFavoriteEntity {
  @PrimaryGeneratedColumn({ type: 'bigint', unsigned: true })
  id: number;

...
export const useUserStore = defineStore('user', () => {
  ...
  const login = async (credentials) => {
    try {
      // 模拟登录：手机号 13800138000，密码任意
      if (credentials.phone === '13800138000') {
        const mockData = {
      ...
      // 真实登录
      const data = await request.post('/app/user/login/password', {
  ...
  const getUserInfo = async () => {
    try {
      const data = await request.get('/app/user/info')
      userInfo.value = data
      return data
    } catch (error) {
      throw error

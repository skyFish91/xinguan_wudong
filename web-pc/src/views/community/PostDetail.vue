      ...
      <div class="detail-container">
        ...
        <div class="post-content">
          ...
          <div class="author-card card">
            <div class="author-info" @click="$router.push(`/user/${post.userId}`)">
              <el-avatar :size="48" :src="post.userAvatar" />
          ...
          <div class="post-topics" v-if="post.topics && post.topics.length">
            <el-tag
              ...
              type="primary"
              class="topic-tag"
              @click="$router.push(`/community?topicId=${topic.id}`)"
            >
              # {{ topic.name }}
            </el-tag>
          </div>
            ...
            <el-dropdown @command="handleMoreAction" style="margin-left: auto">
              ...
              <template #dropdown>
                <el-dropdown-menu>
                  ...
                  <el-dropdown-item command="delete" v-if="isMyPost">
                    删除
                  </el-dropdown-item>
                </el-dropdown-menu>
              </template>
...
const loadComments = async (reset = false) => {
  ...
  try {
    ...
    const data = await communityApi.getComments(route.params.id, {
...
const loadRecommendPosts = () => {
  ...
  recommendPosts.value = Array.from({ length: 5 }, (_, i) => ({
    id: i + 100,
    title: `推荐游记 ${i + 1}`,
    cover: `https://via.placeholder.com/200x150/764ba2/ffffff?text=游记${i + 1}`,

...
| 实施方式 | 1 组 6 人协作开发，多仓库 + npm 私有包（分工详见 §13） |
...
| 角色 | 重点章节 |
|---|---|
| 成员A（组长·架构/公共层） | §2 工程结构、§3.3 公共层 DDL、§4.2 公共接口、§5-§8 |
| 成员B/C/D（后端） | §2 目录树、§3.4 对应模块 DDL、§4.3 对应模块接口、§6 关键实现 |
| 成员E/F（前端） | §2.4 前端目录树、§4 接口、§7 配置、§8 部署 |
| 全体 | §4.1 通用约定、§4.2 公共接口（登录/订单/支付必读）、§13 分工与环境搭建、§14 路演答辩 |

### 0.2 阶段阅读路径

- **Phase 0（公共基础层）**：先读 §2 → §3.3（公共层 DDL）→ §4.2（公共接口）→ §5-§7。
- **Phase 1-5（业务模块）**：读 §2 目录树 → §3.4（本模块 DDL）→ §4.3（本模块接口）。
- **开工前（全员必读）**：先读 §13（分工 + 环境搭建 + 各环节步骤）→ 装好开发环境 → 再按上面路径读各自章节。
...
uni-app小程序 │ Vue3 PC网页 │ Cool Admin Vue 管理后台
   ...
   ├─ module-clothing / food / housing / travel / community   （业务模块 npm 包，成员B/C/D）
   ├─ module-admin   （后台模块 npm 包：商家审核/看板/财务/运营，成员D）
...
| 仓库 | 产出物 | 负责 |
|---|---|---|
| `wudong-common` | npm `@wudong/common` → Verdaccio | 成员A |
| `wudong-module-clothing` | npm `@wudong/module-clothing` | 成员B |
| `wudong-module-food` | npm `@wudong/module-food` | 成员B |
| `wudong-module-housing` | npm `@wudong/module-housing` | 成员C |
| `wudong-module-travel` | npm `@wudong/module-travel` | 成员C |
| `wudong-module-community` | npm `@wudong/module-community` | 成员D |
| `wudong-module-admin` | npm `@wudong/module-admin` | 成员D |
| `wudong-server` | Cool Admin Midway 基础工程（聚合装配） | 成员A |
| `wudong-app` | uni-app 小程序 | 成员E |
| `wudong-web` | Vue3 PC 网页 | 成员F |
| `wudong-admin` | Cool Admin Vue 管理后台 | 成员F（+各成员业务页） |
| `wudong-fe-common` | npm `@wudong/fe-common` | 成员E |

### 2.2 私有 npm 源（Verdaccio）

- 本地/云服务器起 Verdaccio，作为团队私有 npm 源，发布 `@wudong/*` 作用域包。
- 各成员在 `.npmrc` 配置：`registry=http://<私有源>/` 与 `@wudong:registry=http://<私有源>/`。
- 版本管理：`@wudong/common` 发版本 → 各成员模块 `npm install @wudong/common@latest`；联调前锁定版本。
...
```
@wudong/module-clothing/              # 成员B（其余模块同构）
...
└── src/                              # 基于 Cool Admin Vue 二次开发
    ├── views/                        # 成员F：用户/商家/看板/财务/运营
    └── modules/                      # 各成员业务管理页作为子模块嵌入
```

### 2.5 后端模块契约（npm 包，6 人团队必须遵守）
...
| 阶段 | 周期 | 交付物 | 主要人员 |
|---|---|---|---|
| P0 公共基础层 | 第 1-2 周 | 环境搭建、Verdaccio 私有源、@wudong/common（实体/错误码/工具）、wudong-server 骨架、MySQL/Redis Docker、登录鉴权、OSS 上传 | 成员A 牵头 + 全员 |
| P1 业务模块 | 第 2-5 周 | 衣/食/住/行/社区各模块 CRUD + 收藏/评价 + 后台 CRUD | 成员B/C/D 并行 |
| P2 订单支付闭环 | 第 3-6 周 | 统一订单中心、5 类型下单、微信支付(mock)、退款、电子票 | 成员A + 各模块配合 |
| P3 三端前端 | 第 4-7 周 | uni-app 小程序、PC web、Cool Admin 后台页面 | 成员E/F 并行 |
| P4 三端联调 | 第 7-8 周 | 前后端打通、接口联调、bug 修复 | 全员 |
| P5 部署验收 | 第 8 周 | 部署上线、按需求 13.x 验收、提交 DDL/演示 | 全员 |
| P6 路演答辩 | 第 8 周末 | PPT、演示脚本、答辩彩排（§14） | 全员 |

> 6 人分工明细、各阶段具体步骤与环境搭建见 §13；P0 未冻结前各模块先按契约 mock 数据。
...
| 多人并行接口契约漂移 | 联调 | @wudong/common 统一 DTO/错误码；定期对齐契约（§13.4 协作规范） |
...
| V3.1 | 2026-09-08 | 修正为 1 组 6 人协作；新增 §13 团队分工/环境搭建/各环节步骤、§14 路演答辩准备 |

---

## 13. 团队协作：6 人分工、环境搭建与各环节步骤

> 本章是**实操手册**：先定分工（13.1），再搭环境（13.2），然后按阶段步骤推进（13.3），用协作规范兜底（13.4）。阶段编号对应 §10 里程碑。

### 13.1 6 人分工

一个 6 人小组要完成 3 端 + 6 模块 + 公共层 + 订单支付，**不能按"1 人 1 模块"切**——前端 3 端、公共层、订单支付都需要专门人手。推荐按"后端按模块聚类 + 前端按端 + 公共层专人"切：

| 成员 | 角色定位 | 主要负责 | 交付物 |
|---|---|---|---|
| 成员A（组长） | 架构 + 后端核心 | `@wudong/common`（统一用户/鉴权/购物车/上传/消息/搜索/错误码）、统一订单中心 + 微信支付 + 退款、`wudong-server` 骨架、数据库设计统筹、Docker 部署、进度与答辩统筹 | common 包、server 骨架、订单支付闭环、部署脚本 |
| 成员B | 后端 | 衣（商品/SKU/库存）+ 食（餐厅/菜品/餐位/农产品），含收藏/评价 | clothing/food 模块包 |
| 成员C | 后端 | 住（民宿/房型/房态/预订）+ 行（景区/票种/路线/电子票），含收藏/评价 | housing/travel 模块包 |
| 成员D | 后端 | 社区（游记/评论/话题/点赞/关注/举报）+ 管理后台后端（商家审核/看板/财务/运营） | community/admin 模块包 |
| 成员E | 前端 | 小程序端（uni-app 全部模块页面）+ 前端公共库 `@wudong/fe-common` | app 小程序 + fe-common 包 |
| 成员F | 前端 | PC 网页（Vue3）+ 管理后台前端（Cool Admin Vue） | web + admin 前端 |

**分工要点：**

- 成员A 的公共层最重也最关键（登录、下单、支付贯穿所有模块），前期（P0）全员配合成员A：B/C/D 先写各自模块 Entity/DDL，E/F 先搭前端工程与公共组件，等公共层接口冻结后并行推进。
- 订单支付是横切关注点，由成员A 定义契约（下单接口签名、状态机、回调幂等，§4.3/§6.1/§6.3），各业务模块只按契约提交"请求 + 类型专属字段"。
- 前端人手紧：uni-app 与 PC 共用 `@wudong/fe-common`（请求封装、TS 类型、公共组件）；Cool Admin Vue 自带 RBAC/CRUD 界面，后台前端大量工作是"配 CRUD 页面"而非从零写，减轻成员F 负担。
- 进度有快慢时可在成员间临时借调（如 D 完成社区后支援 B 的食模块），由成员A 每周里程碑对齐时统筹。

### 13.2 开发环境搭建（全员，开工第一步）

#### 13.2.1 前置软件清单

| 软件 | 版本要求 | 用途 | 验证命令 |
|---|---|---|---|
| Node.js | 18+（建议 20/22 LTS） | 后端/前端运行时 | `node -v` |
| npm | 随 Node（可选 pnpm 提速） | 包管理 | `npm -v` |
| Git | 2.x | 版本管理 | `git --version` |
| MySQL | 8.0 | 关系数据库 | Docker 或 `mysql --version` |
| Redis | 7.x | 缓存/购物车/token | `redis-cli ping` → `PONG` |
| Docker Desktop | 可选（推荐） | 一键起 MySQL/Redis | `docker --version` |
| VS Code | 最新 | 编辑器 | — |
| 微信开发者工具 | 最新 | 小程序调试 | — |
| HBuilderX 或 uni-app CLI | 最新 | uni-app 开发 | — |

**VS Code 推荐插件**：ESLint、Prettier、Volar（Vue3）、TypeScript、GitLens。

#### 13.2.2 安装步骤（按顺序）

1. **装 Node.js**：nodejs.org 下载 LTS 版安装；装完 `node -v`、`npm -v` 能打印版本即成功。
2. **配 npm 国内镜像**（强烈建议，加速下载）：
   ```bash
   npm config set registry https://registry.npmmirror.com
   ```
3. **装 pnpm（可选）**：`npm install -g pnpm`，验证 `pnpm -v`。
4. **装 Git**：git-scm.com 安装；`git config --global user.name` / `user.email` 配置身份。
5. **起 MySQL 8 + Redis 7**（推荐 Docker，免手动安装）：
   - 安装并启动 Docker Desktop；
   - 在项目根建 `docker-compose.yml`（见 13.2.3）；
   - `docker compose up -d` 后 `docker compose ps` 看到两个容器 running。
   - 不用 Docker 时：本地装 MySQL 8（建库 `wudong`，utf8mb4）与 Redis 7。
6. **跑通两个基座模板**（已 clone 到 `code/` 目录）：
   ```bash
   # 后端基座（Cool Admin Midway）
   cd code/cool-admin-midway && npm install && npm run dev
   # 后台前端基座（Cool Admin Vue）
   cd code/cool-admin-vue && npm install && npm run dev
   ```
7. **验证后端连库**：改 `src/config/config.default.ts` 里 typeorm 的 host/port/username/password/database，重启后日志无报错、能登录后台即通过。
8. **建前端两个工程**（供成员E/F）：
   - uni-app：`npx degit dcloudio/uni-preset-vue#vite wudong-app`
   - PC：`npm create vite@latest wudong-web -- --template vue-ts`，再装 Element Plus。

#### 13.2.3 docker-compose.yml 样例
...
> 本地开发各成员也可各自起一套 MySQL/Redis（默认端口 3306/6379）；若共用一个开发库，提前约定库名 `wudong` 与统一账号，避免互相覆盖。开发期 TypeORM `synchronize: true` 自动建表（§0.4）。

### 13.3 各环节步骤（按 §10 里程碑展开）

> 每个阶段给"目标 → 步骤 checklist → 验收标准 → 负责人"，步骤用 `- [ ]` 勾选，每完成一条提交一次 git。

#### Phase 0 — 公共基础层（第 1-2 周，成员A 牵头 + 全员）

- **目标**：搭好环境与骨架，冻结公共层接口契约，登录鉴权 + 上传跑通，各模块能按契约 mock。
- **步骤**：
  - [ ] 全员完成 §13.2 环境搭建，`wudong-server`、后台前端本地跑通（第 1 周前半）。
  - [ ] 成员A 建 `wudong-server` 骨架：引入 Cool Admin Midway 结构、配 typeorm/redis、`configuration.ts` 注册模块。
  - [ ] 成员A 建 `@wudong/common`：统一响应 `Result`/错误码/异常过滤器/分页工具（§4.2、§6）。
  - [ ] 成员A 写公共层 DDL：`user/merchant/address/order/.../sys_config`（§3.3）。
  - [ ] 成员A 实现登录鉴权（手机验证码 + 微信 mock）与 OSS 上传（§5）。
  - [ ] 成员B/C/D 并行写各自模块 Entity + DDL（§3.4），本地 `synchronize` 建表核对。
  - [ ] 成员E/F 建前端工程 + `@wudong/fe-common` 请求封装（§2.4）。
  - [ ] 全组对齐：冻结公共接口契约（DTO/错误码/路径），成员A 发布 `@wudong/common`。
- **验收**：后端能启动、能登录拿到 token、上传能返回 URL；各模块 Entity 与 DDL 一致；前端能调通一个公共接口。

#### Phase 1 — 业务模块（第 2-5 周，成员B/C/D 并行）

- **目标**：五模块 + 后台的 CRUD、收藏/评价、业务规则完成。
- **步骤（每个模块通用，以衣为例）**：
  - [ ] 写 Entity（商品分类/商品/SKU/图片）+ 建表。
  - [ ] 写 Service（列表/详情/库存扣减）+ Controller（REST 接口，§4.3）。
  - [ ] 写 DTO 校验（`@midwayjs/validate`）+ 单元测试。
  - [ ] 写收藏/评价接口（`clothing_favorite/clothing_review`）。
  - [ ] 接统一上传传图、接搜索 mock。
  - [ ] 后台管理页（成员F 用 Cool Admin 配 CRUD）。
- **验收**：模块接口 Swagger 可查、单测覆盖率 ≥ 60%、后台能增删改查。

#### Phase 2 — 订单支付闭环（第 3-6 周，成员A + 各模块配合）

- **目标**：统一下单（5 类）、支付回调、退款、电子票跑通。
- **步骤**：
  - [ ] 成员A 实现订单中心：`order` 主表 + 5 类型子表 + 状态机（§4.3、§6.1）。
  - [ ] 成员A 实现统一下单接口（按类型路由写入子表）。
  - [ ] 成员A 实现微信支付适配层（真实沙箱 + mock 可切换）+ 回调验签/幂等（§6.3）。
  - [ ] 成员A 实现退款单 + 退款回调 + 财务冲销。
  - [ ] 各模块提供"下单所需参数"（如住模块房态锁定、行模块电子票生成）。
  - [ ] 购物车下单（衣/食特产）、餐位预订下单（食）打通。
- **验收**：能下单 → 支付(mock) → 回调 → 订单 PAID →（部分）退款；金额/状态正确、幂等。

#### Phase 3 — 三端前端（第 4-7 周，成员E/F 并行）

- **目标**：小程序、PC、后台三端页面 + 对接数据。
- **步骤**：
  - [ ] 成员E 写小程序：登录/首页/商品列表/详情/购物车/下单/支付/订单列表/我的收藏评价。
  - [ ] 成员F 写 PC：首页/模块列表详情/下单；后台：各模块 CRUD 页 + 商家审核/看板。
  - [ ] 前后端对接：token、统一 Result、分页、错误提示（§4.1/§4.2）。
  - [ ] 关键链路（登录 → 下单 → 支付 → 订单）走通。
- **验收**：三端能完成一个端到端主流程。

#### Phase 4 — 三端联调（第 7-8 周，全员）

- **目标**：三端 + 全部模块 + 订单支付完整打通，修 bug。
- **步骤**：
  - [ ] 按需求验收清单逐条过（需求 §13.x）。
  - [ ] 修联调发现的接口/契约/状态机问题。
  - [ ] 回归主流程 + 边界（库存不足/房态冲突/取消退款）。
- **验收**：验收清单全部通过。

#### Phase 5 — 部署验收（第 8 周，全员）

- **目标**：云服务器部署上线，交付物齐备。
- **步骤**：
  - [ ] 成员A 写 Docker Compose（server + MySQL + Redis + Nginx）+ Nginx 反代/静态托管（§8）。
  - [ ] 生产 `synchronize:false` + 迁移，配置 HTTPS/域名（备案）。
  - [ ] 提交 DDL、Swagger 文档、单测报告、演示账号。
- **验收**：线上可访问三端，核心流程可演示。

#### Phase 6 — 路演答辩（第 8 周末，全员，见 §14）

### 13.4 协作规范

- **分支策略**：`main`（可运行）+ `feat/<模块或功能>`（个人开发）；合并前由成员A 或互相 review。
- **提交规范**：`type(scope): message`，如 `feat(clothing): 商品列表接口`。
- **接口契约**：以本文档 §4 + `@wudong/common` 的 DTO/错误码为准；改动契约须在群内同步并更新本文档。
- **每日站会（10 分钟）**：每人说"昨天做了什么 / 今天做什么 / 卡在哪"；成员A 记录并解阻塞。
- **每周里程碑对齐**：对照 §10 检查是否落后，落后则临时借调人手。
- **联调冻结**：进入 P4 后锁定 `@wudong/common` 版本，接口不再大改。

---

## 14. 路演答辩准备

> 路演 = 演示 + 讲解 + 问答。目标是让评委 10 分钟内看懂"解决了什么问题、系统怎么架构、我们怎么分工做出来、有哪些亮点"。

### 14.1 演示脚本（建议 6-8 分钟）

| 环节 | 时长 | 内容 | 主讲 |
|---|---|---|---|
| 开场 | 30s | 一句话说清项目：乌东文旅"衣食住行"一站式平台，解决"游客去哪买/吃/住/玩 + 商家线上化 + 平台监管" | 成员A |
| 痛点与定位 | 1min | 3 个痛点 → 我们的 3 端 6 模块解决方案 | 成员A |
| 架构讲解 | 1min | 一张架构图：Midway.js 单体 + Cool Admin + 3 端 + 公共层（§1.2） | 成员A |
| 核心演示 | 3min | 走主流程：登录 → 逛(衣/食/住/行) → 下单 → 支付(mock) → 订单 → 商家核销/评价 | 成员A 操作 + 各成员旁白 |
| 亮点模块 | 1.5min | 选 2-3 个亮点：订单状态机（5 类统一）、房态并发控制、电子票核销、社区内容审核 | 对应成员 |
| 分工与技术难点 | 1min | 6 人怎么分工、遇到的技术难点及解决（§13.1） | 成员A |
| 收尾 | 30s | 总结 + 未来展望（真实支付/推荐算法/更多模块） | 成员A |

**演示要点**：提前录好 1 段 1 分钟主流程视频兜底（现场环境可能不稳）；准备演示账号（游客 + 商家 + 管理员）；每条流程准备"如果卡住"的 Plan B。

### 14.2 PPT 结构（约 12-15 页）

1. 封面（项目名 + 小组成员）
2. 项目背景与痛点
3. 解决方案总览（3 端 6 模块）
4. 技术架构图（§1.2）
5. 数据库设计（表关系 / 订单中心 1+N）
6. 核心功能演示（截图/录屏，分模块）
7. 技术亮点（订单状态机 / 并发控制 / 幂等 / 内容审核）
8. 团队分工（§13.1 表格）
9. 项目进度与里程碑（§10）
10. 测试与质量（覆盖率 / 验收）
11. 演示视频 / 现场演示
...
**原则**：每人讲自己负责的部分，讲清"我做了什么、怎么做的、难点是什么"，避免一人包办。

### 14.4 答辩常见问题与应答要点

| 评委可能问 | 应答要点 |
|---|---|
| 为什么选 Midway.js 而不是 Spring Boot 或其它 Node 框架？ | 阿里系企业级、TS 类型安全、装饰器/IoC 与 Spring 类似易上手、配套 Cool Admin 后台开箱即用（§1.1） |
| 6 个人怎么保证接口不打架？ | 公共层 `@wudong/common` 统一 DTO/错误码 + 模块契约 7 条（§2.5）+ 每日站会（§13.4） |
| 订单为什么设计 1 主表 + 5 子表？ | 统一状态机/支付/退款复用，类型专属字段存子表，跨类型统计和财务只需看主表（§3.3/§4.3） |
| 库存/房态并发怎么防超卖？ | 条件 UPDATE 乐观扣减（`WHERE stock >= n`）+ 房态 `uk_room_date` 唯一约束（§6.6） |
| 微信支付没有企业资质怎么办？ | 预留适配层 + mock，接口签名不变，资质就绪切换真实（§11） |
| 数据安全（密码/手机号/身份证）？ | bcrypt 加密密码、身份证/手机号脱敏、内容审核/举报（需求 §5.9） |
| 项目还能怎么扩展？ | 真实支付/短信、推荐算法、小程序直播带货、商家版小程序、多租户 |

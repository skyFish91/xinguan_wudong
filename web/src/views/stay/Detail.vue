...
async function load() {
  try {
    homestay.value = await request.get(`/app/stay/homestays/${route.params.id}`);
...
async function calcPrice() {
  ...
  try {
    priceResult.value = await request.get('/app/stay/price', {
      params: {
...
async function submitBooking() {
  ...
  booking.value = true;
  try {
    const order: any = await request.post('/app/stay/bookings', {
